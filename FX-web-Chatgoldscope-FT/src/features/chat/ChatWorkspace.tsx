...
const ANALYSIS_STEP_INTERVAL_MS =
  1_250;
...
const quickActions = [
  {
    ...
    description:
      "Understand the current Gold trend, structure and important price areas.",
    ...
    prompt:
      "What is the current XAU/USD market outlook?",
  },

  {
    ...
    description:
      "Review your position, entry, stop loss, target and current risk.",

    meta:
      "Position · Risk · Invalidation",

    tone:
      "success" as const,

    prompt:
      "I want to review my current XAU/USD trade.",
  },
] as const;
      ...
      storedConversation
        ...
        .map(
          (
            ...
            /*
             * Blob preview URLs cannot survive
             * page reloads.
             */
            attachments:
              [],
...
export function ChatWorkspace() {
            ...
            conversation.messages
              .map(
                (
                  ...
                  /*
                   * Local blob URLs are intentionally
                   * not persisted.
                   */
                  attachments:
                    [],
  ...
  /*
   * Revoke any remaining local blob URL
...
const handleAttachmentChange =
  ...
  (
    ...
    if (
      ...
      /*
       * Normal text paste ko untouched rehne do.
       */
      return;
    }
    ...
    /*
     * Clipboard screenshots often have generic
     * names such as "image.png".
     *
     * Give them a useful unique name.
     */
    const extension =
      pastedFile.type ===
      "image/jpeg"
    ...
    const fileName =
      `goldscope-screenshot-${Date.now()}.${extension}`;
  ...
  const handleSubmit =
    async () => {
      ...
      const finalMessage =
        trimmedMessage ||
        "Please analyse this screenshot.";

      /*
       * Keep a local snapshot of the selected
       * attachment for this request.
       */
...
/*
 * Clear the composer immediately after the
 * message has been queued.
 *
 * Do NOT revoke the preview URL here because
 * the sent user message is still rendering
 * the same local image preview.
 */
setAttachment(
  null,
);
...
setIsResponding(
  true,
      ...
      try {
        ...
        /*
         * Do not call clearAttachment() here,
         * because it revokes the blob URL that
         * is currently being used by the sent
         * user-message preview.
         *
         * The preview remains valid for this
         * browser session.
         */
           } catch (
        requestError
...
>
  ...
  const renderConversationalMessage =
    (
      chatMessage:
        ChatMessage,
    ) => {
  ...
  const renderMessage =
    (
      ...
      if (
        ...
        return renderConversationalMessage(
          chatMessage,
        );
      }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-[var(--surface-soft)] rounded-2xl rounded-br-md bg-[var(--primary)] px-4 py-3 text-base leading-7 text-white shadow-[var(--shadow-sm)]">
...
/>
          ...
          <p className="mt-2 text-center text-[11px] leading-5 text-[var(--text-subtle)]">
            GoldScope provides market
            decision-support, not financial
            advice. Verify live execution
            conditions and manage your own risk.
          </p>
        </div>
      </div>
    </div>
